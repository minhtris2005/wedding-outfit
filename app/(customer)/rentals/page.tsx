...
const StatusBadge = ({ status }: { status: string }) => {
  ...
  return (
    <span
      className={`px-4 py-1.5 rounded-full text-[10px] uppercase tracking-widest font-medium border ${config.class}`}
    >
      {config.label}
    </span>
  );
...
export default function RentalsPage() {
  ...
  const fetchMyRentals = useCallback(async () => {
    setLoading(true);
    try {
      const data = await apiFetch<any>("/rentals/my");
      setRentals(Array.isArray(data) ? data : []);
      setTimeout(() => setPageReady(true), 300);
    } catch (error: any) {
      console.error("Lỗi tải đơn thuê:", error);
      if (
  ...
  if (loading || !pageReady) {
    return <LoadingSpinner message="Đang tải đơn thuê của bạn..." />;
  }
          ...
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
            <div>
              ...
              <p className="text-sm text-neutral-500 mt-4 max-w-2xl">
                ...
                Dưới đây là danh sách các đơn thuê váy của bạn.
              </p>
            </div>
          </div>
        ...
        {filteredRentals.length === 0 ? (
          <div className="py-32 text-center bg-white/50 backdrop-blur-sm rounded-3xl border border-dashed border-neutral-200">
            ...
            <p className="text-xs text-neutral-400 uppercase tracking-wider mb-8">
              Hãy chọn những mẫu váy yêu thích và tạo đơn thuê ngay
            </p>
            <Link
              href="/collections"
              className="inline-block bg-black text-white px-8 py-4 text-[10px] uppercase tracking-[0.3em] hover:bg-blue-600 transition-all duration-500 rounded-full shadow-lg"
            >
              Khám phá bộ sưu tập
            </Link>
          </div>
          ...
          <div className="grid gap-6">
            {filteredRentals.map((r, index) => (
              <div
                key={r.id}
                className="group bg-white/80 backdrop-blur-sm rounded-2xl shadow-sm hover:shadow-xl border border-neutral-100 hover:border-blue-200 transition-all duration-500 overflow-hidden animate-slideUp"
                style={{ animationDelay: `${index * 100}ms` }}
              >
                <div className="flex flex-col md:flex-row items-stretch">
                  {/* Image Section */}
                  <div className="md:w-48 relative overflow-hidden bg-gradient-to-br from-neutral-50 to-blue-50/30">
                    <div className="relative aspect-[4/5] md:aspect-auto md:h-full w-full">
                      <Image
                        src={r.dress?.mainImage || "/placeholder.jpg"}
                        alt={r.dress?.name || "Dress image"}
                        fill
                  ...
                  <div className="flex-1 p-8">
                    ...
                    <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-6">
                      <div>
                        ...
                        <p className="text-xs text-neutral-400 flex items-center gap-2">
                          ...
                          Đặt lúc:{" "}
                          {r.createdAt ? formatLocalDateTime(r.createdAt) : ""}
                        </p>
                      </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 py-6 border-t border-b border-neutral-100">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm font-medium text-neutral-800">
                            {r.rentStart ? formatLocalDate(r.rentStart) : ""}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm font-medium text-neutral-800">
                            {r.rentEnd ? formatLocalDate(r.rentEnd) : ""}
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="mt-6 flex items-center justify-end">
                      <Link
                        href={`/dresses/${r.dress?.id}`}
                        className="group/link flex items-center gap-2 text-[10px] uppercase tracking-[0.2em] font-medium text-black hover:text-blue-600 transition-colors"
                      >
